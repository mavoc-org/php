...
if(!function_exists('_esc')) {
    function _esc($value, $double_encode = true) {   
        if($value instanceof \DateTime) {
            $format = 'Y-m-d H:i:s';
            $format = ao()->hook('helper_esc_format', $format);
...
if(!function_exists('stripStyle')) {
    function stripStyle($input) {
        $output = $input;
